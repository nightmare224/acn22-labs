...
for i in range(num_servers - 1):
    for j in range(i + 1, num_servers):
        path_length = dijkstra_fattree.get_path_length(ft_topo.servers[i], ft_topo.servers[j])
...
for _ in range(3):
    ...
    for i in range(num_servers - 1):
        for j in range(i + 1, num_servers):
            path_length = dijkstra_jellyfish.get_path_length(jf_topo.servers[i], jf_topo.servers[j])
...
fig = go.Figure(
    data=[
        go.Bar(name="Jellyfish", x=x_path_length, y=[jf_path_length_count[i]/sum(jf_path_length_count.values()) for i in range(2,7)]),
        go.Bar(name="Fat-tree", x=x_path_length, y=[ft_path_length_count[i]/sum(ft_path_length_count.values()) for i in range(2,7)]),
    ]
)
fig.update_layout(
...
fig.write_html('reproduce_1c.html')
fig.show()
